...
export function generateSchemaMarkup({
  ...
  copa,
  footballTeam,
}: IPartidos): WithContext<SportsEvent> {
  const {
    vs,
    footballStadium,
    img: { src },
  ...
  return {
    ...
    name:
      isVisitor ?
        `Partido de Fútbol: ${vs} vs Estudiantes de La Plata`
      : `Partido de Fútbol: Estudiantes de La Plata vs ${vs}`,
    startDate: date || '',
    eventStatus:
      isSuspended ? 'https://schema.org/EventPostponed' : 'https://schema.org/EventScheduled',
    location: {
      '@type': 'Place',
      name:
        isVisitor ?
...
export function getSchemaScripts(partidos: Array<IPartidos>): string {
